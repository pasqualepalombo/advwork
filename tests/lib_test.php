...
/**
 * Unit tests for mod/advwork/lib.php.
 *
 * @package    mod_advwork
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_advwork_lib_testcase extends advanced_testcase {

    /**
     * Test calendar event provide action open.
     */
    public function test_advwork_core_calendar_provide_event_action_open() {
        $this->resetAfterTest();
        $this->setAdminUser();

        $now = time();
        $course = $this->getDataGenerator()->create_course();
        $advwork = $this->getDataGenerator()->create_module('advwork', ['course' => $course->id,
            'submissionstart' => $now - DAYSECS, 'submissionend' => $now + DAYSECS]);
        $event = $this->create_action_event($course->id, $advwork->id, advwork_EVENT_TYPE_SUBMISSION_OPEN);

        $factory = new \core_calendar\action_factory();
        $actionevent = mod_advwork_core_calendar_provide_event_action($event, $factory);

        $this->assertInstanceOf('\core_calendar\local\event\value_objects\action', $actionevent);
        $this->assertEquals(get_string('viewadvworksummary', 'advwork'), $actionevent->get_name());
     ...
     */
    public function test_advwork_core_calendar_provide_event_action_closed() {
        $this->resetAfterTest();
        $this->setAdminUser();

        $course = $this->getDataGenerator()->create_course();
        $advwork = $this->getDataGenerator()->create_module('advwork', array('course' => $course->id,
     ...
     */
    public function test_advwork_core_calendar_provide_event_action_open_in_future() {
        $this->resetAfterTest();
        $this->setAdminUser();
     ...
     * @throws coding_exception
     */
    public function test_advwork_core_calendar_provide_event_action_no_time_specified() {
        $this->resetAfterTest();
        $this->setAdminUser();
     ...
     * @param int $instanceid The advwork id.
     * @param string $eventtype The event type. eg. advwork_EVENT_TYPE_OPEN.
     * @return bool|calendar_event
     */
    private function create_action_event($courseid, $instanceid, $eventtype) {
        $event = new stdClass();
        $event->name = 'Calendar event';
        $event->modulename = 'advwork';
    ...
    public function test_check_updates_since() {
        ...
        $advwork = $this->getDataGenerator()->create_module('advwork', $record);
        $cm = get_coursemodule_from_instance('advwork', $advwork->id, $course->id);
        ...
        $updates = advwork_check_updates_since($cm, $onehourago);
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        // Submission.
        $submissionid = $generator->create_submission($advwork->id, $student->id, array(
