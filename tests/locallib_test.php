...
/**
 * Unit tests for advwork api class defined in mod/advwork/locallib.php
 *
 * @package    mod_advwork
 * @category   phpunit
...
global $CFG;
require_once($CFG->dirroot . '/mod/advwork/locallib.php'); // Include the code to test
require_once(__DIR__ . '/fixtures/testable.php');


/**
 * Test cases for the internal advwork api
 */
class mod_advwork_internal_api_testcase extends advanced_testcase {
    ...
    protected function setUp() {
        ...
        $advwork = $this->getDataGenerator()->create_module('advwork', array('course' => $this->course));
        $cm = get_coursemodule_from_instance('advwork', $advwork->id, $this->course->id, false, MUST_EXIST);
        $this->advwork = new testable_advwork($advwork, $cm, $this->course);
    }
    ...
    public function test_aggregate_submission_grades_process_notgraded() {
        ...
        // exercise SUT
        $this->advwork->aggregate_submission_grades_process($batch);
    }
    ...
    public function test_aggregate_grading_grades_process_nograding() {
        ...
        //$DB->expectNever('update_record');
        // excersise SUT
        $this->advwork->aggregate_grading_grades_process($batch);
    }
    ...
    public function test_aggregate_grading_grades_process_single_grade_new() {
        ...
        $expected->advworkid = $this->advwork->id;
        $expected->userid = 3;
        $expected->gradinggrade = 82.87670;
        $expected->timegraded = $now;
        //$DB->expectOnce('insert_record', array('advwork_aggregations', $expected));
        // excersise SUT
        $this->advwork->aggregate_grading_grades_process($batch, $now);
    }
    ...
    public function test_percent_to_value() {
        ...
        $percent = 56.6543;
        // exercise SUT
        $part = advwork::percent_to_value($percent, $total);
        // verify
    ...
    public function test_lcm() {
        ...
        $this->assertEquals(advwork::lcm(1,4), 4);
        $this->assertEquals(advwork::lcm(2,4), 4);
        $this->assertEquals(advwork::lcm(4,2), 4);
        $this->assertEquals(advwork::lcm(2,3), 6);
        $this->assertEquals(advwork::lcm(6,4), 12);
    }
    ...
    public function test_lcm_array() {
        ...
        // excersise SUT
        $lcm = array_reduce($numbers, 'advwork::lcm', 1);
        // verify
    ...
    public function test_prepare_example_assessment() {
        ...
        // excersise SUT
        $a = $this->advwork->prepare_example_assessment($fakerawrecord);
        // verify
        $this->assertTrue($a instanceof advwork_example_assessment);
    ...
    public function test_prepare_example_reference_assessment() {
        ...
        // excersise SUT
        $a = $this->advwork->prepare_example_reference_assessment($fakerawrecord);
        // verify
        $this->assertTrue($a instanceof advwork_example_reference_assessment);
    ...
    public function test_user_restrictions() {
        ...
        $courseid = $this->advwork->course->id;
        ...
        // The existing advwork doesn't have any restrictions, so user lists
        // should include all three users.
        $allusers = get_enrolled_users(context_course::instance($courseid));
        $result = $this->advwork->get_grouped($allusers);
        ...
        $users = $this->advwork->get_potential_authors(false);
        $this->assertCount(3, $users);
        $users = $this->advwork->get_potential_authors(false, $group2->id);
        $this->assertEquals(array($student2->id), array_keys($users));

        // Create another test advwork with grouping set.
        $advworkitem = $this->getDataGenerator()->create_module('advwork',
                array('course' => $courseid, 'groupmode' => SEPARATEGROUPS,
                'groupingid' => $grouping->id));
        $cm = get_coursemodule_from_instance('advwork', $advworkitem->id,
                $courseid, false, MUST_EXIST);
        $advworkgrouping = new testable_advwork($advworkitem, $cm, $this->advwork->course);

        // This time the result should only include users and groups in the
        // selected grouping.
        $result = $advworkgrouping->get_grouped($allusers);
        ...
        // Test get_users_with_capability_sql (via get_potential_authors).
        $users = $advworkgrouping->get_potential_authors(false);
        $userids = array_keys($users);
        sort($userids);
        $this->assertEquals(array($student1->id, $student2->id), $userids);
        $users = $advworkgrouping->get_potential_authors(false, $group2->id);
        $this->assertEquals(array($student2->id), array_keys($users));

        // Enable the availability system and create another test advwork with
        ...
        $cm = get_coursemodule_from_instance('advwork', $advworkitem->id,
                $courseid, false, MUST_EXIST);
        $advworkrestricted = new testable_advwork($advworkitem, $cm, $this->advwork->course);

        // The get_grouped function isn't intended to apply this restriction,
        // so it should be the same as the base advwork. (Note: in reality,
        // get_grouped is always run with the parameter being the result of
        // one of the get_potential_xxx functions, so it works.)
        $result = $advworkrestricted->get_grouped($allusers);
        $this->assertCount(4, $result);
        $this->assertCount(3, $result[0]);

        // The get_users_with_capability_sql-based functions should apply it.
        $users = $advworkrestricted->get_potential_authors(false);
        $userids = array_keys($users);
        sort($userids);
        $this->assertEquals(array($student1->id, $student2->id), $userids);
        $users = $advworkrestricted->get_potential_authors(false, $group2->id);
        $this->assertEquals(array($student2->id), array_keys($users));
    }

    /**
     * Test the advwork reset feature.
     */
    public function test_reset_phase() {
        $this->resetAfterTest(true);

        $this->advwork->switch_phase(advwork::PHASE_CLOSED);
        $this->assertEquals(advwork::PHASE_CLOSED, $this->advwork->phase);

        $settings = (object)array(
            'reset_advwork_phase' => 0,
        );
        $status = $this->advwork->reset_userdata($settings);
        $this->assertEquals(advwork::PHASE_CLOSED, $this->advwork->phase);
        ...
        $status = $this->advwork->reset_userdata($settings);
        $this->assertEquals(advwork::PHASE_SETUP, $this->advwork->phase);
    ...
    /**
     * Test deleting assessments related data on advwork reset.
     */
    public function test_reset_userdata_assessments() {
        ...
        $subid2 = $advworkgenerator->create_submission($this->advwork->id, $student2->id);

        $asid1 = $advworkgenerator->create_assessment($subid1, $student2->id);
        $asid2 = $advworkgenerator->create_assessment($subid2, $student1->id);
        ...
        $this->assertEquals(2, $DB->count_records('advwork_submissions', array('advworkid' => $this->advwork->id)));
        $this->assertEquals(0, $DB->count_records('advwork_assessments'));
    }

    /**
     * Test deleting submissions related data on advwork reset.
     */
    public function test_reset_userdata_submissions() {
        ...
        $this->assertEquals(0, $DB->count_records('advwork_submissions', array('advworkid' => $this->advwork->id)));
        $this->assertEquals(0, $DB->count_records('advwork_assessments'));
    }
    ...
    public function test_normalize_file_extensions() {
        ...
        advwork::normalize_file_extensions('');
    ...
    public function test_invalid_file_extensions() {
        ...
        advwork::invalid_file_extensions('', '');
    ...
    public function test_is_allowed_file_type() {
        ...
        advwork::is_allowed_file_type('', '');
        $this->assertDebuggingCalledCount(2);
    }

    /**
     * Test advwork::check_group_membership() functionality.
     */
    public function test_check_group_membership() {
        ...
        // advwork with no groups.
        $advworkitem1 = $this->getDataGenerator()->create_module('advwork', [
            'course' => $courseid,
            'groupmode' => NOGROUPS,
        ]);
        $cm = get_coursemodule_from_instance('advwork', $advworkitem1->id, $courseid, false, MUST_EXIST);
        $advwork1 = new testable_advwork($advworkitem1, $cm, $this->course);

        $this->setUser($teacher1);
        $this->assertTrue($advwork1->check_group_membership($student1->id));
        $this->assertTrue($advwork1->check_group_membership($student2->id));
        $this->assertTrue($advwork1->check_group_membership($student3->id));
        ...
        // advwork in visible groups mode.
        $advworkitem2 = $this->getDataGenerator()->create_module('advwork', [
            'course' => $courseid,
            'groupmode' => VISIBLEGROUPS,
        ]);
        $cm = get_coursemodule_from_instance('advwork', $advworkitem2->id, $courseid, false, MUST_EXIST);
        $advwork2 = new testable_advwork($advworkitem2, $cm, $this->course);

        $this->setUser($teacher1);
        $this->assertTrue($advwork2->check_group_membership($student1->id));
        $this->assertTrue($advwork2->check_group_membership($student2->id));
        $this->assertTrue($advwork2->check_group_membership($student3->id));
        ...
        // advwork in separate groups mode.
        $advworkitem3 = $this->getDataGenerator()->create_module('advwork', [
            'course' => $courseid,
            'groupmode' => SEPARATEGROUPS,
        ]);
        $cm = get_coursemodule_from_instance('advwork', $advworkitem3->id, $courseid, false, MUST_EXIST);
        $advwork3 = new testable_advwork($advworkitem3, $cm, $this->course);

        $this->setUser($teacher1);
        $this->assertTrue($advwork3->check_group_membership($student1->id));
        $this->assertTrue($advwork3->check_group_membership($student2->id));
        $this->assertTrue($advwork3->check_group_membership($student3->id));

        $this->setUser($tutor1);
        $this->assertTrue($advwork3->check_group_membership($student1->id));
        $this->assertFalse($advwork3->check_group_membership($student2->id));
        $this->assertFalse($advwork3->check_group_membership($student3->id));

        $this->setUser($tutor2);
        $this->assertFalse($advwork3->check_group_membership($student1->id));
        $this->assertTrue($advwork3->check_group_membership($student2->id));
        $this->assertFalse($advwork3->check_group_membership($student3->id));
    }
}
