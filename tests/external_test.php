...
/**
 * advwork module external functions tests
 *
 * @package    mod_advwork
 * @category   external
 ...
 * @since      Moodle 3.4
 */
class mod_advwork_external_testcase extends externallib_advanced_testcase {

    /** @var stdClass course object */
    private $course;
    /** @var stdClass advwork object */
    private $advwork;
    ...
    public function setUp() {
        ...
        $this->advwork = $this->getDataGenerator()->create_module('advwork',
            array(
                'course' => $this->course->id,
                'overallfeedbackfiles' => 1,
            )
        );
        $this->context = context_module::instance($this->advwork->cmid);
        $this->cm = get_coursemodule_from_instance('advwork', $this->advwork->id);

        // Add grading strategy data (accumulative is the default).
        $advwork = new advwork($this->advwork, $this->cm, $this->course);
    ...
    /**
     * Test test_mod_advwork_get_advworks_by_courses
     */
    public function test_mod_advwork_get_advworks_by_courses() {
        ...
        $advwork2 = self::getDataGenerator()->create_module('advwork', $record);
        ...
        $returndescription = mod_advwork_external::get_advworks_by_courses_returns();

        // Create what we expect to be returned when querying the two courses.
        $properties = advwork_summary_exporter::read_properties_definition();
        $expectedfields = array_keys($properties);

        // Add expected coursemodule and data.
        $advwork1 = $this->advwork;
        $advwork1->coursemodule = $advwork1->cmid;
        $advwork1->introformat = 1;
        $advwork1->introfiles = [];
        $advwork1->instructauthorsfiles = [];
        $advwork1->instructauthorsformat = 1;
        $advwork1->instructreviewersfiles = [];
        $advwork1->instructreviewersformat = 1;
        $advwork1->conclusionfiles = [];
        $advwork1->conclusionformat = 1;

        $advwork2->coursemodule = $advwork2->cmid;
        $advwork2->introformat = 1;
        $advwork2->introfiles = [];
        $advwork2->instructauthorsfiles = [];
        $advwork2->instructauthorsformat = 1;
        $advwork2->instructreviewersfiles = [];
        $advwork2->instructreviewersformat = 1;
        $advwork2->conclusionfiles = [];
        $advwork2->conclusionformat = 1;

        foreach ($expectedfields as $field) {
            if (!empty($properties[$field]) && $properties[$field]['type'] == PARAM_BOOL) {
                $advwork1->{$field} = (bool) $advwork1->{$field};
                $advwork2->{$field} = (bool) $advwork2->{$field};
            }
            $expected1[$field] = $advwork1->{$field};
            $expected2[$field] = $advwork2->{$field};
        }

        $expectedadvworks = array($expected2, $expected1);

        // Call the external function passing course ids.
        $result = mod_advwork_external::get_advworks_by_courses(array($course2->id, $this->course->id));
        $result = external_api::clean_returnvalue($returndescription, $result);

        $this->assertEquals($expectedadvworks, $result['advworks']);
        $this->assertCount(0, $result['warnings']);

        // Call the external function without passing course id.
        $result = mod_advwork_external::get_advworks_by_courses();
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedadvworks, $result['advworks']);
        $this->assertCount(0, $result['warnings']);

        // Unenrol user from second course and alter expected advworks.
        $enrol->unenrol_user($instance2, $this->student->id);
        array_shift($expectedadvworks);
        ...
        $result = mod_advwork_external::get_advworks_by_courses(array($course2->id));
    ...
    /**
     * Test mod_advwork_get_advwork_access_information for students.
     */
    public function test_mod_advwork_get_advwork_access_information_student() {

        self::setUser($this->student);
        $result = mod_advwork_external::get_advwork_access_information($this->advwork->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_advwork_access_information_returns(), $result);
        ...
        unassign_capability('mod/advwork:peerassess', $this->studentrole->id);
        unassign_capability('mod/advwork:submit', $this->studentrole->id);
        unset($enabledcaps[0]);
        unset($enabledcaps[1]);
        ...
        // Switch phase.
        $advwork = new advwork($this->advwork, $this->cm, $this->course);
        $advwork->switch_phase(advwork::PHASE_SUBMISSION);
        $result = mod_advwork_external::get_advwork_access_information($this->advwork->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_advwork_access_information_returns(), $result);
    ...
    /**
     * Test mod_advwork_get_advwork_access_information for teachers.
     */
    public function test_mod_advwork_get_advwork_access_information_teacher() {
    ...
    /**
     * Test mod_advwork_get_user_plan for students.
     */
    public function test_mod_advwork_get_user_plan_student() {

        self::setUser($this->student);
        $result = mod_advwork_external::get_user_plan($this->advwork->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_user_plan_returns(), $result);

        $this->assertCount(0, $result['userplan']['examples']);  // No examples given.
        $this->assertCount(5, $result['userplan']['phases']);  // Always 5 phases.
        $this->assertEquals(advwork::PHASE_SETUP, $result['userplan']['phases'][0]['code']);  // First phase always setup.
        $this->assertTrue($result['userplan']['phases'][0]['active']); // First phase "Setup" active in new advworks.
        ...
        $this->assertEquals(advwork::PHASE_SUBMISSION, $result['userplan']['phases'][1]['code']);
        $this->assertTrue($result['userplan']['phases'][1]['active']); // We are now in submission phase.
    }

    /**
     * Test mod_advwork_get_user_plan for teachers.
     */
    public function test_mod_advwork_get_user_plan_teacher() {
        ...
        $this->assertEquals(advwork::PHASE_SETUP, $result['userplan']['phases'][0]['code']);  // First phase always setup.
        $this->assertTrue($result['userplan']['phases'][0]['active']); // First phase "Setup" active in new advworks.
        $this->assertCount(4, $result['userplan']['phases'][0]['tasks']);  // For new empty advworks, always 4 tasks.
    ...
    /**
     * Test test_view_advwork invalid id.
     */
    public function test_view_advwork_invalid_id() {
        $this->expectException('moodle_exception');
        mod_advwork_external::view_advwork(0);
    }

    /**
     * Test test_view_advwork user not enrolled.
     */
    public function test_view_advwork_user_not_enrolled() {
        // Test not-enrolled user.
        $usernotenrolled = self::getDataGenerator()->create_user();
        $this->setUser($usernotenrolled);
        $this->expectException('moodle_exception');
        mod_advwork_external::view_advwork($this->advwork->id);
    }

    /**
     * Test test_view_advwork user student.
     */
    public function test_view_advwork_user_student() {
        ...
        $result = mod_advwork_external::view_advwork($this->advwork->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::view_advwork_returns(), $result);
        ...
        $this->assertInstanceOf('\mod_advwork\event\course_module_viewed', $event);
        $this->assertEquals($this->context, $event->get_context());
        $moodleadvwork = new \moodle_url('/mod/advwork/view.php', array('id' => $this->cm->id));
        $this->assertEquals($moodleadvwork, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
    }

    /**
     * Test test_view_advwork user missing capabilities.
     */
    public function test_view_advwork_user_missing_capabilities() {
        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        assign_capability('mod/advwork:view', CAP_PROHIBIT, $this->studentrole->id, $this->context->id);
    ...
    public function test_add_submission() {
        ...
        $result = mod_advwork_external::add_submission($this->advwork->id, $title, $content, FORMAT_MOODLE, $draftidinlineattach,
            $draftidattach);
        $result = external_api::clean_returnvalue(mod_advwork_external::add_submission_returns(), $result);
        $this->assertEmpty($result['warnings']);

        // Check submission created.
        $submission = $advwork->get_submission_by_author($this->student->id);
        $this->assertTrue($result['status']);
        ...
        // Check files.
        $contentfiles = $fs->get_area_files($this->context->id, 'mod_advwork', 'submission_content', $submission->id);
        ...
        $contentfiles = $fs->get_area_files($this->context->id, 'mod_advwork', 'submission_attachment', $submission->id);
        $this->assertCount(2, $contentfiles);
        foreach ($contentfiles as $file) {
            if ($file->is_directory()) {
                continue;
    ...
    public function test_add_submission_invalid_phase() {
        ...
        $this->expectException('moodle_exception');
        mod_advwork_external::add_submission($this->advwork->id, 'Test');
    }
    ...
    public function test_add_submission_empty_title() {
        ...
        $this->expectException('moodle_exception');
        mod_advwork_external::add_submission($this->advwork->id, '');
    }
    ...
    public function test_add_submission_already_added() {
        ...
        $result = mod_advwork_external::add_submission($this->advwork->id, 'My submission');
        $result = external_api::clean_returnvalue(mod_advwork_external::add_submission_returns(), $result);
    ...
    public function test_update_submission() {
        ...
        $result = mod_advwork_external::update_submission($submissionid, $title, $content, FORMAT_MOODLE, $draftidinlineattach,
            $draftidattach);
        $result = external_api::clean_returnvalue(mod_advwork_external::update_submission_returns(), $result);
        $this->assertEmpty($result['warnings']);

        // Check submission updated.
        $advwork = new advwork($this->advwork, $this->cm, $this->course);
        $submission = $advwork->get_submission_by_id($submissionid);
        $this->assertTrue($result['status']);
        $this->assertEquals($title, $submission->title);
        $this->assertEquals($content, $submission->content);
    ...
    public function test_update_submission_of_other_user() {
        ...
        $this->expectException('moodle_exception');
        mod_advwork_external::update_submission($submissionid, 'Test');
    }
    ...
    public function test_update_submission_empty_title() {
        ...
        $this->expectException('moodle_exception');
        mod_advwork_external::update_submission($submissionid, '');
    }
    ...
    public function test_delete_submission() {
        ...
        $result = mod_advwork_external::delete_submission($submissionid);
        $result = external_api::clean_returnvalue(mod_advwork_external::delete_submission_returns(), $result);
        $this->assertEmpty($result['warnings']);
        $this->assertTrue($result['status']);
        $advwork = new advwork($this->advwork, $this->cm, $this->course);
        $submission = $advwork->get_submission_by_author($this->student->id);
        ...
        // Checking event.
        $this->assertInstanceOf('\mod_advwork\event\submission_deleted', $event);
        $this->assertEquals($this->context, $event->get_context());
    }
    ...
    public function test_delete_submission_with_assessments() {
        ...
        $advworkgenerator->create_assessment($submissionid, $this->teacher->id, array(
            'weight' => 3,
            'grade' => 95.00000,
        ));
        ...
        $this->setUser($this->student);
        $this->expectException('moodle_exception');
        mod_advwork_external::delete_submission($submissionid);
    }
    ...
    public function test_get_submissions_student() {
        ...
        $this->setUser($this->student);
        $result = mod_advwork_external::get_submissions($this->advwork->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submissions_returns(), $result);
    ...
    public function test_get_submissions_published_student() {
        ...
        $advwork = new advwork($this->advwork, $this->cm, $this->course);
        $advwork->switch_phase(advwork::PHASE_CLOSED);
        // Create a couple of submissions with files.
        $advworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        $submission = array('published' => 1);
        $submissionid = $advworkgenerator->create_submission($this->advwork->id, $this->anotherstudentg1->id, $submission);
    ...
    public function test_get_submissions_from_student_with_feedback_from_teacher() {
        ...
        // Create a couple of submissions with files.
        $advworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        $submissionid = $advworkgenerator->create_submission($this->advwork->id, $this->student->id);
        ...
        // Remove teacher caps.
        assign_capability('mod/advwork:viewallsubmissions', CAP_PROHIBIT, $this->teacher->id, $this->context->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
        course_modinfo::clear_instance_cache();

        $this->setUser($this->teacher);
        $result = mod_advwork_external::get_submissions($this->advwork->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submissions_returns(), $result);
        // We should get just our submission.
        $this->assertEquals(1, $result['totalcount']);
        $this->assertEquals($submissionid, $result['submissions'][0]['id']);
    ...
    public function test_get_submissions_from_students_as_teacher() {
        ...
        // Create a couple of submissions with files.
        $advworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        $submissionid1 = $advworkgenerator->create_submission($this->advwork->id, $this->student->id);
        $submissionid2 = $advworkgenerator->create_submission($this->advwork->id, $this->anotherstudentg1->id);
        $submissionid3 = $advworkgenerator->create_submission($this->advwork->id, $this->anotherstudentg2->id);

        $this->setUser($this->teacher);
        $result = mod_advwork_external::get_submissions($this->advwork->id); // Get all.
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submissions_returns(), $result);
        $this->assertEquals(3, $result['totalcount']);
        $this->assertCount(3, $result['submissions']);

        $result = mod_advwork_external::get_submissions($this->advwork->id, 0, 0, 0, 2); // Check pagination.
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submissions_returns(), $result);
        $this->assertEquals(3, $result['totalcount']);
        $this->assertCount(2, $result['submissions']);

        $result = mod_advwork_external::get_submissions($this->advwork->id, 0, $this->group2->id); // Get group 2.
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submissions_returns(), $result);
        $this->assertEquals(1, $result['totalcount']);
        $this->assertCount(1, $result['submissions']);
        $this->assertEquals($submissionid3, $result['submissions'][0]['id']);

        $result = mod_advwork_external::get_submissions($this->advwork->id, $this->anotherstudentg1->id); // Get one.
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submissions_returns(), $result);
        $this->assertEquals(1, $result['totalcount']);
    ...
    public function test_get_submission_student() {
        ...
        $advwork->switch_phase(advwork::PHASE_CLOSED);
        $this->setUser($this->student);
        $result = mod_advwork_external::get_submission($firstsubmissionid);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submission_returns(), $result);
        ...
        $advwork->switch_phase(advwork::PHASE_EVALUATION);
        $result = mod_advwork_external::get_submission($firstsubmissionid);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submission_returns(), $result);
        $this->assertEquals($firstsubmissionid, $result['submission']['id']);
        $this->assertCount(1, $result['submission']['contentfiles']); // Check we retrieve submission text files.
    ...
    public function test_get_submission_i_reviewed() {
        ...
        $firstsubmissionid = $this->create_test_submission($this->student);  // Create submission with files.
        $advworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        $advworkgenerator->create_assessment($firstsubmissionid, $this->anotherstudentg1->id, array(
            'weight' => 3,
            'grade' => 95,
        ));
    ...
    public function test_get_submission_published_student() {
        ...
        $this->setUser($this->student);
        $result = mod_advwork_external::get_submission($submissionid);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submission_returns(), $result);
        ...
        $this->setUser($this->anotherstudentg2);
        $this->expectException('moodle_exception');
        mod_advwork_external::get_submission($submissionid);
    }
    ...
    public function test_get_submission_from_student_with_feedback_from_teacher() {
        ...
        $advwork->switch_phase(advwork::PHASE_EVALUATION);
        unassign_capability('mod/advwork:viewallsubmissions', $this->teacherrole->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
    ...
    public function test_get_submission_from_students_as_teacher() {
        ...
        $this->setUser($this->teacher);
        $result = mod_advwork_external::get_submission($submissionid1); // Get all.
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submission_returns(), $result);
        $this->assertEquals($submissionid1, $result['submission']['id']);

        $result = mod_advwork_external::get_submission($submissionid3); // Get group 2.
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submission_returns(), $result);
    ...
    public function test_get_submission_assessments_student() {
        ...
        $advworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        $advworkgenerator->create_assessment($submissionid, $this->anotherstudentg1->id, array(
            'weight' => 3,
            'grade' => 95,
        ));
        $advworkgenerator->create_assessment($submissionid, $this->student->id, array(
            'weight' => 2,
            'grade' => 90,
        ));
        ...
        $advwork->switch_phase(advwork::PHASE_CLOSED);
        $this->setUser($this->student);
        $result = mod_advwork_external::get_submission_assessments($submissionid);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_submission_assessments_returns(), $result);
    ...
    public function test_get_submission_assessments_invalid_phase() {
        ...
        $this->expectException('moodle_exception');
        mod_advwork_external::get_submission_assessments($submissionid);
    }
    ...
    public function test_get_submission_assessments_teacher() {
        ...
        $advworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        $assessmentid = $advworkgenerator->create_assessment($submissionid, $this->anotherstudentg1->id, array(
            'weight' => 1,
            'grade' => 50,
        ));
    ...
    public function test_get_assessment_author() {
        ...
        $advworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        $assessmentid = $advworkgenerator->create_assessment($submissionid, $this->student->id, array(
            'weight' => 2,
            'grade' => 90,
        ));
        ...
        $advwork->switch_phase(advwork::PHASE_CLOSED);
        $this->setUser($this->anotherstudentg1);
        $result = mod_advwork_external::get_assessment($assessmentid);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_assessment_returns(), $result);
    ...
    public function test_get_assessment_student_invalid_phase() {
        ...
        $this->expectException('moodle_exception');
        mod_advwork_external::get_assessment($assessmentid);
    }
    ...
    public function test_get_assessment_form_definition_reviewer_new_assessment() {
        ...
        $advwork = new advwork($this->advwork, $this->cm, $this->course);
        $submission = $advwork->get_submission_by_id($submissionid);
        $assessmentid = $advwork->add_allocation($submission, $this->student->id);

        // Switch to assessment phase.
        $advwork->switch_phase(advwork::PHASE_ASSESSMENT);
        $this->setUser($this->student);
        $result = mod_advwork_external::get_assessment_form_definition($assessmentid);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_assessment_form_definition_returns(), $result);
    ...
    public function test_get_assessment_form_definition_invalid_phase() {
        ...
        $advwork = new advwork($this->advwork, $this->cm, $this->course);
        $submission = $advwork->get_submission_by_id($submissionid);
        $assessmentid = $advwork->add_allocation($submission, $this->anotherstudentg1->id);

        $advwork->switch_phase(advwork::PHASE_EVALUATION);
        $this->setUser($this->student);
        // Since we are not reviewers we can't see the assessment until the advwork is closed.
        $this->expectException('moodle_exception');
        mod_advwork_external::get_assessment_form_definition($assessmentid);
    }
    ...
    public function test_get_reviewer_assessments() {
        ...
        $advworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_advwork');
        $assessmentid1 = $advworkgenerator->create_assessment($submissionid1, $this->student->id, array(
            'weight' => 2,
            'grade' => 90,
        ));
        $assessmentid2 = $advworkgenerator->create_assessment($submissionid2, $this->student->id, array(
            'weight' => 3,
            'grade' => 80,
        ));
        ...
        $this->setUser($this->student);
        // Get my assessments.
        $result = mod_advwork_external::get_reviewer_assessments($this->advwork->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_reviewer_assessments_returns(), $result);
        ...
        $result = mod_advwork_external::get_reviewer_assessments($this->advwork->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_reviewer_assessments_returns(), $result);
        $this->assertCount(2, $result['assessments']);
    }
    ...
    public function test_get_reviewer_assessments_other_student() {
        ...
        $this->setUser($this->student);
        $this->expectException('moodle_exception');
        mod_advwork_external::get_reviewer_assessments($this->advwork->id, $this->anotherstudentg1->id);
    }
    ...
    public function test_update_assessment() {
        ...
        $result = mod_advwork_external::update_assessment($assessmentid, $data);
        $result = external_api::clean_returnvalue(mod_advwork_external::update_assessment_returns(), $result);
    ...
    public function test_get_grades() {
        ...
        advwork_grade_item_update($this->advwork, (object) $submissiongrade, (object) $assessmentgrade);

        // First retrieve my grades.
        $this->setUser($this->student);
        $result = mod_advwork_external::get_grades($this->advwork->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_grades_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        ...
        $this->setUser($this->teacher);
        $result = mod_advwork_external::get_grades($this->advwork->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_grades_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        $this->assertEquals($assessmentgrade['rawgrade'], $result['assessmentrawgrade']);
        $this->assertEquals($submissiongrade['rawgrade'], $result['submissionrawgrade']);
    ...
    public function test_get_grades_other_student() {
        ...
        $this->setUser($this->anotherstudentg1);
        $this->expectException('moodle_exception');
        mod_advwork_external::get_grades($this->advwork->id, $this->student->id);
    }
    ...
    public function test_evaluate_assessment() {
        ...
        $weight = 10;
        $gradinggradeover = 10;
        $result = mod_advwork_external::evaluate_assessment($assessmentid, $feedbacktext, $feedbackformat, $weight,
            $gradinggradeover);
        $result = external_api::clean_returnvalue(mod_advwork_external::evaluate_assessment_returns(), $result);
        $this->assertTrue($result['status']);

        $assessment = $DB->get_record('advwork_assessments', array('id' => $assessmentid));
    ...
    public function test_evaluate_assessment_ignore_parameters() {
        ...
        assign_capability('mod/advwork:allocate', CAP_PROHIBIT, $this->teacherrole->id, $this->context->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
    ...
    public function test_evaluate_assessment_no_permissions() {
        ...
        $gradinggradeover = 50;
        $this->expectException('moodle_exception');
        mod_advwork_external::evaluate_assessment($assessmentid, $feedbacktext, $feedbackformat, $weight, $gradinggradeover);
    }
    ...
    public function test_get_grades_report() {
        ...
        $assessmentid1 = $advworkgenerator->create_assessment($submissionid2, $this->student->id, array(
            'weight' => 100,
            'grade' => 50,
        ));
        $assessmentid2 = $advworkgenerator->create_assessment($submissionid1, $this->anotherstudentg1->id, array(
            'weight' => 100,
            'grade' => 55,
        ));
        ...
        $advwork->switch_phase(advwork::PHASE_CLOSED);
        $this->setUser($this->teacher);
        $result = mod_advwork_external::get_grades_report($this->advwork->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_grades_report_returns(), $result);
        ...
        foreach ($result['report']['grades'] as $grade) {
            if ($grade['userid'] == $this->student->id) {
                ...
                $this->assertEquals($advwork->real_grade(50), $grade['reviewerof'][0]['grade']); // Check grade (converted).
                $this->assertEquals($advwork->real_grade(55), $grade['reviewedby'][0]['grade']); // Check grade (converted).
                ...
                $this->assertEquals($advwork->real_grade(55), $grade['reviewerof'][0]['grade']); // Check grade (converted).
                $this->assertEquals($advwork->real_grade(50), $grade['reviewedby'][0]['grade']); // Check grade (converted).
            }
        }
        // Now check pagination.
        $result = mod_advwork_external::get_grades_report($this->advwork->id, 0, 'lastname', 'ASC', 0, 1);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_grades_report_returns(), $result);
        $this->assertEquals(3, $result['report']['totalcount']); // Expect the total count.
        $this->assertCount(1, $result['report']['grades']);

        // Groups filtering.
        $result = mod_advwork_external::get_grades_report($this->advwork->id, $this->group1->id);
        $result = external_api::clean_returnvalue(mod_advwork_external::get_grades_report_returns(), $result);
    ...
    public function test_get_grades_report_invalid_phase() {
        $this->setUser($this->teacher);
        $this->expectException('moodle_exception');
        $this->expectExceptionMessage(get_string('nothingfound', 'advwork'));
        mod_advwork_external::get_grades_report($this->advwork->id);
    }
    ...
    public function test_view_submission() {
        ...
        $this->setUser($this->student);
        $result = mod_advwork_external::view_submission($firstsubmissionid);
        $result = external_api::clean_returnvalue(mod_advwork_external::view_submission_returns(), $result);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_advwork\event\submission_viewed', $event);
        $this->assertEquals($this->context, $event->get_context());
        $moodleadvwork = new \moodle_url('/mod/advwork/submission.php', array('id' => $firstsubmissionid,
            'cmid' => $this->cm->id));
        $this->assertEquals($moodleadvwork, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
    ...
    public function test_evaluate_submission() {
        ...
        $published = 1;
        $gradeover = 10;
        $result = mod_advwork_external::evaluate_submission($submissionid, $feedbacktext, $feedbackformat, $published,
            $gradeover);
        $result = external_api::clean_returnvalue(mod_advwork_external::evaluate_submission_returns(), $result);
        $this->assertTrue($result['status']);

        $submission = $DB->get_record('advwork_submissions', array('id' => $submissionid));
        $this->assertEquals($feedbacktext, $submission->feedbackauthor);
        $this->assertEquals($advwork->raw_grade_value($gradeover, $advwork->grade), $submission->gradeover);  // Expected grade.
    ...
    public function test_evaluate_submission_no_permissions() {
        ...
        $gradeover = 50;
        $this->expectException('moodle_exception');
        mod_advwork_external::evaluate_submission($submissionid, $feedbacktext, $feedbackformat, $published, $gradeover);
    }
    ...
    public function test_evaluate_submission_invalid_grade() {
        ...
        $published = 1;
        $gradeover = 150;
        $result = mod_advwork_external::evaluate_submission($submissionid, $feedbacktext, $feedbackformat, $published, $gradeover);
        $result = external_api::clean_returnvalue(mod_advwork_external::evaluate_submission_returns(), $result);
        $this->assertCount(1, $result['warnings']);
        $this->assertFalse($result['status']);
