 ...
 *
 * @package    advworkallocation_random
 ...
 * Allocates the submissions randomly
 */
class advwork_random_allocator implements advwork_allocator {
    ...
    public function init() {
        ...
        $this->mform = new advwork_random_allocator_form($PAGE->url, $customdata);
     ...
     *
     * @param advwork_random_allocator_setting $setting
     * @param advwork_allocation_result allocation result logger
     */
    public function execute(advwork_random_allocator_setting $settings, advwork_allocation_result $result) {

        $authors        = $this->advwork->get_potential_authors();
        $authors        = $this->advwork->get_grouped($authors);
        $reviewers      = $this->advwork->get_potential_reviewers(!$settings->assesswosubmission);
        $reviewers      = $this->advwork->get_grouped($reviewers);
        $assessments    = $this->advwork->get_all_assessments();
        ...
        if ($settings->numofreviews) {
            ...
            $result->log(get_string('numofrandomlyallocatedsubmissions', 'advworkallocation_random', count($randomallocations)));
        ...
        if ($settings->addselfassessment) {
            ...
            $result->log(get_string('numofselfallocatedsubmissions', 'advworkallocation_random', count($selfallocations)));
            unset($selfallocations);
        }
        if (empty($newallocations)) {
            $result->log(get_string('noallocationtoadd', 'advworkallocation_random'), 'info');
        } else {
            ...
            foreach ($newallocations as $newallocation) {
                ...
                if (isset($allreviewers[$reviewerid])) {
                    ...
                    if (!$allreviewersreloaded) {
                        $allreviewers = $this->advwork->get_potential_reviewers(false);
                ...
                if (in_array($newallocation, $newnonexistingallocations)) {
                    $result->log(get_string('allocationaddeddetail', 'advworkallocation_random', $a), 'ok', 1);
                } else {
                    $result->log(get_string('allocationreuseddetail', 'advworkallocation_random', $a), 'ok', 1);
                }
            }
        }
        if ($settings->removecurrent) {
            ...
            // by reviewer
            $result->log(get_string('numofdeallocatedassessment', 'advworkallocation_random', count($delassessments)), 'info');
            foreach ($delassessments as $delassessmentkey => $delassessmentid) {
                ...
                if (!is_null($assessments[$delassessmentid]->grade)) {
                    $result->log(get_string('allocationdeallocategraded', 'advworkallocation_random', $a), 'error', 1);
                    unset($delassessments[$delassessmentkey]);
                } else {
                    $result->log(get_string('assessmentdeleteddetail', 'advworkallocation_random', $a), 'info', 1);
                }
            }
            $this->advwork->delete_assessment($delassessments);
        }
        $result->set_status(advwork_allocation_result::STATUS_EXECUTED);
    }
    ...
    public function ui() {
        ...
        $message = new advwork_message();
        if ($m == self::MSG_SUCCESS) {
            $message->set_text(get_string('randomallocationdone', 'advworkallocation_random'));
            $message->set_type(advwork_message::TYPE_OK);
        }
        ...
        $gmode = groups_get_activity_groupmode($this->advwork->cm, $this->advwork->course);
        if (VISIBLEGROUPS == $gmode or SEPARATEGROUPS == $gmode) {
            $users = $this->advwork->get_potential_authors() + $this->advwork->get_potential_reviewers();
            $users = $this->advwork->get_grouped($users);
            if (isset($users[0])) {
                ...
                if (!empty($nogroupusers)) {
                    ...
                    $a = implode(', ', $list);
                    $out .= $output->box(get_string('nogroupusers', 'advworkallocation_random', $a), 'generalbox warning nogroupusers');
                }
            }
        }

        // TODO $out .= $output->heading(get_string('stats', 'advworkallocation_random'));
     ...
     *
     * @param array $authors grouped of {@see advwork::get_potential_authors()}
     * @param array $reviewers grouped by {@see advwork::get_potential_reviewers()}
     * @param array $assessments as returned by {@see advwork::get_all_assessments()}
    ...
    protected function add_new_allocations(array $newallocations, array $dataauthors, array $datareviewers) {
        ...
        $submissions    = $this->advwork->get_submissions($authorids);
        $submissions    = $this->index_submissions_by_authors($submissions);
        foreach ($newallocations as $newallocation) {
            $reviewerid = key($newallocation);
            $authorid = current($newallocation);
            if (!isset($submissions[$authorid])) {
                throw new moodle_exception('unabletoallocateauthorwithoutsubmission', 'advwork');
            }
            $submission = $submissions[$authorid];
            $status = $this->advwork->add_allocation($submission, $reviewerid, 1, true);   // todo configurable weight?
            if (advwork::ALLOCATION_EXISTS == $status) {
        ...
        if (is_array($submissions)) {
            foreach ($submissions as $submissionid => $submission) {
                if (isset($byauthor[$submission->authorid])) {
                    throw new moodle_exception('moresubmissionsbyauthor', 'advwork');
                }
     ...
     * @param advwork_allocation_result $result allocation result logger
    ...
    protected function random_allocation($authors, $reviewers, $assessments, $result, array $options) {
        ...
        if (advwork_random_allocator_setting::NUMPER_SUBMISSION == $numper) {
            // circles are authors, squares are reviewers
            $result->log(get_string('resultnumperauthor', 'advworkallocation_random', $numofreviews), 'info');
            $allcircles = $authors;
            $allsquares = $reviewers;
            // get current workload
            list($circlelinks, $squarelinks) = $this->convert_assessments_to_links($assessments);
        } elseif (advwork_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            $result->log(get_string('resultnumperreviewer', 'advworkallocation_random', $numofreviews), 'info');
            $allcircles = $reviewers;
            $allsquares = $authors;
            // get current workload
            list($squarelinks, $circlelinks) = $this->convert_assessments_to_links($assessments);
        } else {
            throw new moodle_exception('unknownusertypepassed', 'advwork');
        }
                    ...
                    while ($keeptrying && (count($circlelinks[$circleid]) < $requiredreviews)) {
                        ...
                        if (NOGROUPS == $gmode) {
                            if (in_array(0, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeers', 'advworkallocation_random'), 'error', 1);
                                break;
                            }
                            $targetgroup = 0;
                            ...
                            if (in_array($circlegroupid, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeersingroup', 'advworkallocation_random'), 'error', 1);
                                break;
                            }
                        ...
                        if ($targetgroup === false) {
                            $keeptrying = false;
                            $result->log(get_string('resultnotenoughpeers', 'advworkallocation_random'), 'error', 1);
                            break;
                        }
        ...
        if (advwork_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            foreach ($circlelinks as $circleid => $squares) {
                foreach ($squares as $squareid) {
     ...
     *
     * @param array $assessments array of assessments as returned by {@link advwork::get_all_assessments()}
 ...
 */
class advwork_random_allocator_setting {
     ...
     * @return advwork_random_allocator_setting
     */
