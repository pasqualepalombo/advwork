 ...
 */
class advwork_manual_allocator implements advwork_allocator {
    ...
    public function init() {
        ...
        if ($perpage and $perpage > 0 and $perpage <= 1000) {
            require_sesskey();
            set_user_preference('advworkallocation_manual_perpage', $perpage);
            redirect($PAGE->url);
        }
        ...
        case 'new':
            ...
            $submission = $this->advwork->get_submission_by_author($authorid);
            if (!$submission) {
                ...
                $res = $this->advwork->add_allocation($submission, $reviewerid);
                if ($res == advwork::ALLOCATION_EXISTS) {
                    $m[] = self::MSG_EXISTS;
        ...
        case 'del':
            ...
            $assessment     = $this->advwork->get_assessment_by_id($assessmentid);
            if ($assessment) {
                if (!$confirmed) {
                    ...
                    if($this->advwork->delete_assessment($assessment->id)) {
                        $m[] = self::MSG_DELETED;
                        $m[] = $assessment->authorid;
                        $m[] = $assessment->reviewerid;
                    } else {
    ...
    public function ui() {
        ...
        $output     = $PAGE->get_renderer('advworkallocation_manual');

        $page       = optional_param('page', 0, PARAM_INT);
        $perpage    = get_user_preferences('advworkallocation_manual_perpage', 10);
        $groupid    = groups_get_activity_group($this->advwork->cm, true);
        ...
        if ($m) {
            ...
            case self::MSG_ADDED:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new advwork_message(get_string('allocationadded', 'advworkallocation_manual'),
                    advwork_message::TYPE_OK);
                break;
            case self::MSG_EXISTS:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new advwork_message(get_string('allocationexists', 'advworkallocation_manual'),
                    advwork_message::TYPE_INFO);
                break;
            case self::MSG_NOSUBMISSION:
                $hlauthorid     = $m[1];
                $message        = new advwork_message(get_string('nosubmissionfound', 'advwork'),
                    advwork_message::TYPE_ERROR);
                break;
            case self::MSG_CONFIRM_DEL:
                $hlauthorid     = $m[2];
                $hlreviewerid   = $m[3];
                if ($m[4] == 0) {
                    $message    = new advwork_message(get_string('areyousuretodeallocate', 'advworkallocation_manual'),
                        advwork_message::TYPE_INFO);
                } else {
                    $message    = new advwork_message(get_string('areyousuretodeallocategraded', 'advworkallocation_manual'),
                        advwork_message::TYPE_ERROR);
                }
                $url = new moodle_url($PAGE->url, array('mode' => 'del', 'what' => $m[1], 'confirm' => 1, 'sesskey' => sesskey()));
                $label = get_string('iamsure', 'advwork');
                $message->set_action($url, $label);
                break;
            case self::MSG_DELETED:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new advwork_message(get_string('assessmentdeleted', 'advwork'),
                    advwork_message::TYPE_OK);
                break;
            case self::MSG_DELETE_ERROR:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new advwork_message(get_string('assessmentnotdeleted', 'advwork'),
                    advwork_message::TYPE_ERROR);
                break;
            }
        }

        // fetch the list of ids of all advwork participants
        $numofparticipants = $this->advwork->count_participants(false, $groupid);
        $participants = $this->advwork->get_participants(false, $groupid, $perpage * $page, $perpage);
        ...
        if ($hlauthorid > 0 and $hlreviewerid > 0) {
            ...
            $button = $output->single_button($PAGE->url, get_string('showallparticipants', 'advworkallocation_manual'), 'get');
        } else {
            $button = '';
        }
        ...
        $submissions = $this->advwork->get_submissions(array_keys($participants));
        ...
        if ($participants) {
            ...
            $sql = "SELECT a.id AS assessmentid, a.submissionid,
                     ...
                     WHERE u.id $participantids AND s.advworkid = :advworkid AND s.example = 0";
        ...
        $data                   = new advworkallocation_manual_allocations();
        $data->advwork         = $this->advwork;
        $data->allocations      = $allocations;
        $data->userinfo         = $userinfo;
        $data->authors          = $this->advwork->get_potential_authors();
        $data->reviewers        = $this->advwork->get_potential_reviewers();
        $data->hlauthorid       = $hlauthorid;
        $data->hlreviewerid     = $hlreviewerid;
        $data->selfassessment   = $this->advwork->useselfassessment;

        // prepare the group selector
        $groupselector = $output->container(groups_print_activity_menu($this->advwork->cm, $PAGE->url, true), 'groupwidget');
 ...
 *
 * @see advwork_manual_allocator::ui()
 */
class advworkallocation_manual_allocations implements renderable {

    /** @var advwork module instance */
    public $advwork;
