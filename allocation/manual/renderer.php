 ...
 */
class advworkallocation_manual_renderer extends mod_advwork_renderer  {
     ...
     *
     * @param advworkallocation_manual_allocations $data to be displayed
     * @return string html code
     */
    protected function render_advworkallocation_manual_allocations(advworkallocation_manual_allocations $data) {

        $this->advwork     = $data->advwork;
        ...
        $selfassessment     = $data->selfassessment;    // bool is the self-assessment allowed in this advwork?
        ...
        $table->head        = array(get_string('participantreviewedby', 'advwork'),
                                    get_string('participant', 'advwork'),
                                    get_string('participantrevierof', 'advwork'));
    ...
    /**
     * Returns information about the advwork participant
     *
     * @return string HTML code
     */
    protected function helper_participant(stdclass $allocation, array $userinfo) {
        ...
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nosubmissionfound', 'advwork'), 'info');
        } else {
            $link = $this->advwork->submission_url($allocation->submissionid);
            ...
            if (is_null($allocation->submissiongrade)) {
                $o .= $this->output->container(get_string('nogradeyet', 'advwork'), array('grade', 'missing'));
            } else {
                $o .= $this->output->container(get_string('alreadygraded', 'advwork'), array('grade', 'missing'));
            }
        }
    ...
    protected function helper_reviewers_of_participant(stdclass $allocation, array $userinfo, array $reviewers, $selfassessment) {
        $o = '';
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nothingtoreview', 'advwork'), 'info');
        } else {
            $exclude = array();
            if (! $selfassessment) {
            ...
            if ($options) {
                ...
                $select = new single_select($handler, 'by', $options, '', array(''=>get_string('chooseuser', 'advwork')), 'addreviewof' . $allocation->userid);
                $select->set_label(get_string('addreviewer', 'advworkallocation_manual'));
    ...
    protected function helper_reviewees_of_participant(stdclass $allocation, array $userinfo, array $authors, $selfassessment) {
        $o = '';
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('withoutsubmission', 'advwork'), 'info');
        }
        $exclude = array();
        if (! $selfassessment) {
            $exclude[$allocation->userid] = true;
            $o .= $this->output->container(get_string('selfassessmentdisabled', 'advwork'), 'info');
        }
        // todo add an option to exclude users without own submission
        ...
        if ($options) {
            ...
            $select = new single_select($handler, 'of', $options, '', array(''=>get_string('chooseuser', 'advwork')), 'addreviewby' . $allocation->userid);
            $select->set_label(get_string('addreviewee', 'advworkallocation_manual'));
            $o .= $this->output->render($select);
        } else {
            $o .= $this->output->container(get_string('nothingtoreview', 'advwork'), 'info');
        }
