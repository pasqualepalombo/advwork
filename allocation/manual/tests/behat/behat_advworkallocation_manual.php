...
/**
 * Steps definitions related to advworkallocation_manual.
 *
 * @package    advworkallocation_manual
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_advworkallocation_manual extends behat_base {
    /**
     * Manually adds a reviewer for advwork participant.
     *
     * This step should start on manual allocation page.
     *
     * @When /^I add a reviewer "(?P<reviewer_name_string>(?:[^"]|\\")*)" for advwork participant "(?P<participant_name_string>(?:[^"]|\\")*)"$/
     * @param string $reviewername
     * @param string $participantname
     */
    public function i_add_a_reviewer_for_advwork_participant($reviewername, $participantname) {
        ...
        try {
            ...
            $this->find_button(get_string('showallparticipants', 'advworkallocation_manual'))->press();
            $selectnode = $this->find('xpath', $xpathselect);
        }
        ...
        $allocatedtext = behat_context_helper::escape(
            get_string('allocationadded', 'advworkallocation_manual'));
        $this->find('xpath', "//*[contains(.,$allocatedtext)]");
    }

    /**
     * Manually allocates multiple reviewers in advwork.
     *
     * @When /^I allocate submissions in advwork "(?P<advwork_name_string>(?:[^"]|\\")*)" as:"$/
     ...
     */
    public function i_allocate_submissions_in_advwork_as($advworkname, TableNode $table) {

        $this->find_link($advworkname)->click();
        $this->execute('behat_navigation::i_navigate_to_in_current_page_administration', get_string('allocate', 'advwork'));
        $rows = $table->getRows();
        ...
        for ($i = 1; $i < count($rows); $i++) {
            $this->i_add_a_reviewer_for_advwork_participant($rows[$i][$reviewer], $rows[$i][$participant]);
        }
    }
}
