...
/**
 * advwork conversion handler
 */
class moodle1_mod_advwork_handler extends moodle1_mod_handler {

    /** @var array the temporary in-memory cache for the current <MOD> contents */
    protected $currentadvwork = null;

    /** @var array in-memory cache for the course module information for the current advwork  */
    protected $currentcminfo = null;

    /** @var array the mapping of legacy elementno => newelementid for the current advwork */
    protected $newelementids = array();

    /** @var moodle1_file_manager for the current advwork */
    protected $fileman = null;

    /** @var moodle1_inforef_manager */
    protected $inforefman = null;

    /** @var array list of moodle1_advworkform_handler instances */
     ...
     *
     * Note that the path /MOODLE_BACKUP/COURSE/MODULES/MOD/advwork does not
    ...
    public function get_paths() {
        return array(
            new convert_path('advwork', '/MOODLE_BACKUP/COURSE/MODULES/MOD/advwork'),
            new convert_path('advwork_elements', '/MOODLE_BACKUP/COURSE/MODULES/MOD/advwork/ELEMENTS'),
            new convert_path(
                'advwork_element', '/MOODLE_BACKUP/COURSE/MODULES/MOD/advwork/ELEMENTS/ELEMENT',
                array(
                    'dropfields' => array(
                        'stddev',
                        'totalassessments',
            ...
            new convert_path('advwork_element_rubric', '/MOODLE_BACKUP/COURSE/MODULES/MOD/advwork/ELEMENTS/ELEMENT/RUBRICS/RUBRIC'),
        );
    }

    /**
     * This is executed every time we have one /MOODLE_BACKUP/COURSE/MODULES/MOD/advwork
     * data available
     */
    public function process_advwork($data, $raw) {

        // re-use the upgrade function to convert advwork record
        $fakerecord = (object)$data;
        $fakerecord->course = 12345678;
        $this->currentadvwork = (array)advwork_upgrade_transform_instance($fakerecord);
        unset($this->currentadvwork['course']);

        // add the new fields with the default values
        $this->currentadvwork['id']                        = $data['id'];
        $this->currentadvwork['evaluation']                = 'best';
        $this->currentadvwork['examplesmode']              = advwork::EXAMPLES_VOLUNTARY;
        $this->currentadvwork['gradedecimals']             = 0;
        $this->currentadvwork['instructauthors']           = '';
        $this->currentadvwork['instructauthorsformat']     = FORMAT_HTML;
        $this->currentadvwork['instructreviewers']         = '';
        $this->currentadvwork['instructreviewersformat']   = FORMAT_HTML;
        $this->currentadvwork['latesubmissions']           = 0;
        $this->currentadvwork['conclusion']                = '';
        $this->currentadvwork['conclusionformat']          = FORMAT_HTML;

        foreach (array('submissionend', 'submissionstart', 'assessmentend', 'assessmentstart') as $field) {
            if (!array_key_exists($field, $this->currentadvwork)) {
                $this->currentadvwork[$field] = null;
            }
        }
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_advwork');

        // convert course files embedded into the intro
        $this->fileman->filearea = 'intro';
        $this->fileman->itemid   = 0;
        $this->currentadvwork['intro'] = moodle1_converter::migrate_referenced_files($this->currentadvwork['intro'], $this->fileman);

        // write advwork.xml
        $this->open_xml_writer("activities/advwork_{$moduleid}/advwork.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'advwork', 'contextid' => $contextid));
        $this->xmlwriter->begin_tag('advwork', array('id' => $instanceid));

        foreach ($this->currentadvwork as $field => $value) {
            if ($field <> 'id') {
                $this->xmlwriter->full_tag($field, $value);
            }
        }

        return $this->currentadvwork;
    }
     ...
     * will append dimensions data in {@link self::process_advwork_element()}
     */
    public function on_advwork_elements_start() {

        $this->xmlwriter->begin_tag('subplugin_advworkform_'.$this->currentadvwork['strategy'].'_advwork');

        // inform the strategy handler that a new advwork instance is being processed
        $handler = $this->get_strategy_handler($this->currentadvwork['strategy']);
     ...
     */
    public function process_advwork_element($data, $raw) {
     ...
     */
    public function process_advwork_element_rubric($data, $raw) {
        if ($this->currentadvwork['strategy'] == 'rubric') {
     ...
     */
    public function on_advwork_element_end() {
     ...
     * This is executed when the parser reaches </ELEMENTS>
     */
    public function on_advwork_elements_end() {
        // give the strategy hanlders last chance to write what they need
        $handler = $this->get_strategy_handler($this->currentadvwork['strategy']);
        $handler->on_elements_end();

        // close the dimensions definition
        $this->xmlwriter->end_tag('subplugin_advworkform_'.$this->currentadvwork['strategy'].'_advwork');
     ...
     */
    public function on_advwork_end() {
        // close advwork.xml
        $this->xmlwriter->end_tag('advwork');
        ...
        $moduleid = $this->currentcminfo['id'];
        $this->open_xml_writer("activities/advwork_{$moduleid}/inforef.xml");
        $this->inforefman->write_refs($this->xmlwriter);
        $this->close_xml_writer();

        // get ready for the next instance
        $this->currentadvwork = null;
        $this->currentcminfo   = null;
        $this->newelementids   = array();
    }

    /**
     * Provides access to the current <advwork> data
     *
     * @return array|null
     */
    public function get_current_advwork() {
        return $this->currentadvwork;
    }
     ...
     * @return moodle1_advworkform_handler the instance of the handler
     */
    protected function get_strategy_handler($strategy) {
        ...
        if (is_null($this->strategyhandlers)) {
            ...
            $subplugins = core_component::get_plugin_list('advworkform');
            foreach ($subplugins as $name => $dir) {
                $handlerfile  = $dir.'/backup/moodle1/lib.php';
                $handlerclass = "moodle1_advworkform_{$name}_handler";
                ...
                $this->log('preparing advwork grading strategy handler', backup::LOG_DEBUG, $handlerclass);
                $this->strategyhandlers[$name] = new $handlerclass($this, $name);
                if (!$this->strategyhandlers[$name] instanceof moodle1_advworkform_handler) {
        ...
        if (!isset($this->strategyhandlers[$strategy])) {
            throw new moodle1_convert_exception('usupported_subplugin', 'advworkform_'.$strategy);
        }
 ...
 */
abstract class moodle1_advworkform_handler extends moodle1_submod_handler {

    /**
     * @param moodle1_mod_handler $advworkhandler the handler of a module we are subplugin of
     * @param string $subpluginname the name of the subplugin
     */
    public function __construct(moodle1_mod_handler $advworkhandler, $subpluginname) {
        parent::__construct($advworkhandler, 'advworkform', $subpluginname);
    }

    /**
     * Provides a xml_writer instance to this advworkform handler
     *
     ...
     *
     * Gives the handler a chance to prepare for a new advwork instance
     */
...
/**
 * Given a record containing data from 1.9 advwork table, returns object containing data as should be saved in 2.0 advwork table
 *
 * @param stdClass $old record from 1.9 advwork table
 * @return stdClass
 */
function advwork_upgrade_transform_instance(stdClass $old) {
    global $CFG;
    ...
    $new->phase         = advwork::PHASE_CLOSED;
