...
/**
 * Various advwork maintainance utilities
 *
 * @package    mod_advwork
...
$params = array(
    ...
    'courseid' => $course->id,
    'other' => array('advworkid' => $advwork->id)
);

switch ($tool) {
case 'clearaggregatedgrades':
    require_capability('mod/advwork:overridegrades', $advwork->context);
    $advwork->clear_submission_grades();
    $advwork->clear_grading_grades();
    $event = \mod_advwork\event\assessment_evaluations_reset::create($params);
    $event->trigger();
    break;

case 'clearassessments':
    require_capability('mod/advwork:overridegrades', $advwork->context);
    $advwork->clear_assessments();
    $event = \mod_advwork\event\assessments_reset::create($params);
    $event->trigger();
    break;
}
