 ...
 */
class advwork_best_evaluation extends advwork_evaluation {

    /** @var advwork the parent advwork instance */
    protected $advwork;

    /** @var the recently used settings in this advwork */
    protected $settings;

    /**
     * Constructor
     *
     * @param advwork $advwork The advwork api instance
     * @return void
     */
    public function __construct(advwork $advwork) {
        global $DB;
        $this->advwork = $advwork;
        $this->settings = $DB->get_record('advworkeval_best_settings', array('advworkid' => $this->advwork->id));
    }

    /**
     * Calculates the grades for assessment and updates 'gradinggrade' fields in 'advwork_assessments' table
     *
    ...
    public function update_grading_grades(stdclass $settings, $restrict=null) {
        ...
        // Remember the recently used settings for this advwork.
        if (empty($this->settings)) {
            $record = new stdclass();
            $record->advworkid = $this->advwork->id;
            $record->comparison = $settings->comparison;
            $DB->insert_record('advworkeval_best_settings', $record);
        } elseif ($this->settings->comparison != $settings->comparison) {
            $DB->set_field('advworkeval_best_settings', 'comparison', $settings->comparison,
                    array('advworkid' => $this->advwork->id));
        }

        // Get the grading strategy instance.
        $grader = $this->advwork->grading_strategy_instance();
     ...
     *
     * @return advwork_best_evaluation_settings_form
     */
    public function get_settings_form(moodle_url $actionurl=null) {
        ...
        return new advwork_best_evaluation_settings_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public static function delete_instance($advworkid) {
        ...
        $DB->delete_records('advworkeval_best_settings', array('advworkid' => $advworkid));
    }
    ...
    protected function process_assessments(array $assessments, array $diminfo, stdclass $settings) {
        ...
        if (is_null($average)) {
            foreach ($assessments as $asid => $assessment) {
                if (!is_null($assessment->gradinggrade)) {
                    $DB->set_field('advwork_assessments', 'gradinggrade', null, array('id' => $asid));
                }
            }
            return;
        }
        ...
        foreach ($grades as $assessmentid => $grade) {
            if (grade_floats_different($grade, $assessments[$assessmentid]->gradinggrade)) {
                ...
                $DB->update_record('advwork_assessments', $record, true);  // bulk operations expected
            }
        }
 ...
 */
class advwork_best_evaluation_settings_form extends advwork_evaluation_settings_form {
    ...
    protected function definition_sub() {
        ...
        $plugindefaults = get_config('advworkeval_best');
        $current = $this->_customdata['current'];

        $options = array();
        for ($i = 9; $i >= 1; $i = $i-2) {
            $options[$i] = get_string('comparisonlevel' . $i, 'advworkeval_best');
        }
        $mform->addElement('select', 'comparison', get_string('comparison', 'advworkeval_best'), $options);
        $mform->addHelpButton('comparison', 'comparison', 'advworkeval_best');
