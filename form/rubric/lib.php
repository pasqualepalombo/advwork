 ...
 * @return bool
 */
function advworkform_rubric_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('advworkform_rubric', array('id' => $itemid ,'advworkid' => $advwork->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/advworkform_rubric/$filearea/$itemid/$relativepath";
 ...
 */
class advwork_rubric_strategy implements advwork_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        $this->descriptionopts = array_merge(array('context' => $this->advwork->context), $this->descriptionopts);
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $this->advwork->context, 'advworkform_rubric', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new advwork_edit_rubric_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        if ($DB->record_exists('advworkform_rubric_config', array('advworkid' => $this->advwork->id))) {
            $DB->set_field('advworkform_rubric_config', 'layout', $layout, array('advworkid' => $this->advwork->id));
        } else {
            $record = new stdclass();
            $record->advworkid = $this->advwork->id;
            $record->layout     = $layout;
            $DB->insert_record('advworkform_rubric_config', $record, false);
        }
        ...
        foreach ($data as $record) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('advworkform_rubric', $record);
            } else {
                // exiting field
                $DB->update_record('advworkform_rubric', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $this->advwork->context, 'advworkform_rubric', 'description', $record->id);
            $DB->update_record('advworkform_rubric', $record);
            ...
            foreach ($record->levels as $level) {
                ...
                if (empty($level->id)) {
                    // new field
                    $level->id = $DB->insert_record('advworkform_rubric_levels', $level);
                } else {
                    // exiting field
                    $DB->update_record('advworkform_rubric_levels', $level);
                }
            }
        }
        $DB->delete_records_list('advworkform_rubric_levels', 'id', $deletelevs);
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $this->advwork->context->id, 'advworkform_rubric', 'description',
        ...
        $formclassname = 'advwork_rubric_' . $this->config->layout . '_assessment_form';
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id AS id, MIN(l.grade) AS min, MAX(l.grade) AS max, 1 AS weight
                  FROM {advworkform_rubric} d
            INNER JOIN {advworkform_rubric_levels} l ON (d.id = l.dimensionid)
                 WHERE d.advworkid = :advworkid
              GROUP BY d.id';
    ...
    public static function delete_instance($advworkid) {
        ...
        $dimensions = $DB->get_records('advworkform_rubric', array('advworkid' => $advworkid), '', 'id');
        $DB->delete_records_list('advworkform_rubric_levels', 'dimensionid', array_keys($dimensions));
        $DB->delete_records('advworkform_rubric', array('advworkid' => $advworkid));
        $DB->delete_records('advworkform_rubric_config', array('advworkid' => $advworkid));
    }
    ...
    protected function load_fields() {
        ...
        $sql = "SELECT r.id AS rid, r.sort, r.description, r.descriptionformat,
                  ...
                  FROM {advworkform_rubric} r
             LEFT JOIN {advworkform_rubric_levels} l ON (l.dimensionid = r.id)
                 WHERE r.advworkid = :advworkid
    ...
    protected function load_config() {
        ...
        if (!$config = $DB->get_record('advworkform_rubric_config', array('advworkid' => $this->advwork->id), 'layout')) {
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($this->advwork->context->id, 'advworkform_rubric', 'description', $id);
            }
        }
        $DB->delete_records_list('advworkform_rubric', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link advwork_edit_rubric_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        for ($i = 0; $i < $raw->norepeats; $i++) {
            ...
            $cook[$i]->advworkid           = $this->advwork->id;
            $cook[$i]->sort                 = $i + 1;
