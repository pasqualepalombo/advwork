 ...
 * @uses moodleform
 */
class advwork_edit_rubric_strategy_form extends advwork_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'advworkform_rubric', $i+1));
            ...
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                                get_string('dimensiondescription', 'advworkform_rubric'), '', $descriptionopts);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $mform->addGroup($levelgrp, 'level__idx_'.$i.'__idy_'.$j, get_string('levelgroup', 'advworkform_rubric'), array(' '), false);
        ...
        $mform->addElement('submit', 'adddims', get_string('addmoredimensions', 'advworkform_rubric',
                advwork_rubric_strategy::ADDDIMS));
        $mform->closeHeaderBefore('adddims');

        $mform->addElement('header', 'configheader', get_string('configuration', 'advworkform_rubric'));
        $layoutgrp = array();
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutlist', 'advworkform_rubric'), 'list');
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutgrid', 'advworkform_rubric'), 'grid');
        $mform->addGroup($layoutgrp, 'layoutgrp', get_string('layout', 'advworkform_rubric'), array('<br />'), false);
    ...
    protected function validation_inner($data, $files) {
        ...
        for ($i = 0; isset($data['dimensionid__idx_'.$i]); $i++) {
            ...
            if (!$atleastonelevel) {
                $errors['level__idx_'.$i.'__idy_0'] = get_string('mustdefinelevel', 'advworkform_rubric');
            }
        }
