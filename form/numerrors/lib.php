 ...
 * @return bool
 */
function advworkform_numerrors_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('advworkform_numerrors', array('id' => $itemid ,'advworkid' => $advwork->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/advworkform_numerrors/$filearea/$itemid/$relativepath";
 ...
 */
class advwork_numerrors_strategy implements advwork_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'advworkform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new advwork_edit_numerrors_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->numerrors; // data to be saved into {advworkform_numerrors}
        $mappings   = $data->mappings;  // data to be saved into {advworkform_numerrors_map}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('advworkform_numerrors', $record);
            } else {
                // exiting field
                $DB->update_record('advworkform_numerrors', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts, $PAGE->context,
                                                      'advworkform_numerrors', 'description', $record->id);
            $DB->update_record('advworkform_numerrors', $record);
        }
        ...
        foreach ($data->mappings as $nonegative => $grade) {
            ...
            if (isset($this->mappings[$nonegative])) {
                $DB->set_field('advworkform_numerrors_map', 'grade', $grade,
                            array('advworkid' => $this->advwork->id, 'nonegative' => $nonegative));
            } else {
                $DB->insert_record('advworkform_numerrors_map',
                            (object)array('advworkid' => $this->advwork->id, 'nonegative' => $nonegative, 'grade' => $grade));
            }
        }
        ...
        $sql = "DELETE FROM {advworkform_numerrors_map}
                      WHERE (($insql nonegative > :maxnonegative) AND (advworkid = :advworkid))";
        $params['maxnonegative'] = $maxnonegative;
        $params['advworkid']   = $this->advwork->id;
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'advworkform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new advwork_numerrors_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function get_dimensions_info() {
        ...
        $dimrecords = $DB->get_records('advworkform_numerrors', array('advworkid' => $this->advwork->id), 'sort', 'id,weight');
    ...
    public static function delete_instance($advworkid) {
        ...
        $DB->delete_records('advworkform_numerrors', array('advworkid' => $advworkid));
        $DB->delete_records('advworkform_numerrors_map', array('advworkid' => $advworkid));
    }
    ...
    protected function load_mappings() {
        global $DB;
        return $DB->get_records('advworkform_numerrors_map', array('advworkid' => $this->advwork->id), 'nonegative',
                                'nonegative,grade'); // we can use nonegative as key here as it must be unique within advwork
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        $fs         = get_file_storage();
        foreach ($ids as $id) {
            $fs->delete_area_files($PAGE->context->id, 'advworkform_numerrors', 'description', $id);
        }
        $DB->delete_records_list('advworkform_numerrors', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link advwork_edit_numerrors_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->numerrors    = array();          // to be stored in {advworkform_numerrors}
        $cook->mappings     = array();          // to be stored in {advworkform_numerrors_map}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->numerrors[$i]                        = new stdclass();
            $cook->numerrors[$i]->id                    = $raw->{'dimensionid__idx_'.$i};
            $cook->numerrors[$i]->advworkid            = $this->advwork->id;
     ...
     * @return array of filtered records from the table advwork_grades
     */
    ...
    protected function update_peer_grade(stdclass $assessment) {
        ...
        if (!is_null($suggested)) {
            $this->advwork->set_peer_grade($assessment->id, $suggested);
        }
        return $suggested;
    }
