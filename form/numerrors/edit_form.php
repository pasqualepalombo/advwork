 ...
 * @uses moodleform
 */
class advwork_edit_numerrors_strategy_form extends advwork_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $plugindefaults     = get_config('advworkform_numerrors');
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'advworkform_numerrors', $i+1));
            $mform->addElement('hidden', 'dimensionid__idx_'.$i);   // the id in advwork_forms
            $mform->setType('dimensionid__idx_'.$i, PARAM_INT);
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                    get_string('dimensiondescription', 'advworkform_numerrors'), '', $descriptionopts);
            $mform->addElement('text', 'grade0__idx_'.$i, get_string('grade0', 'advworkform_numerrors'), array('size'=>'15'));
            $mform->setDefault('grade0__idx_'.$i, $plugindefaults->grade0);
            $mform->setType('grade0__idx_'.$i, PARAM_TEXT);
            $mform->addElement('text', 'grade1__idx_'.$i, get_string('grade1', 'advworkform_numerrors'), array('size'=>'15'));
            ...
            $mform->addElement('select', 'weight__idx_'.$i,
                    get_string('dimensionweight', 'advworkform_numerrors'), advwork::available_dimension_weights_list());
            $mform->setDefault('weight__idx_'.$i, 1);
        }

        $mform->addElement('header', 'mappingheader', get_string('grademapping', 'advworkform_numerrors'));
        $mform->addElement('static', 'mappinginfo', get_string('maperror', 'advworkform_numerrors'),
                                                            get_string('mapgrade', 'advworkform_numerrors'));
        ...
        $percents[''] = '';
        for ($i = 100; $i >= 0; $i--) {
            $percents[$i] = get_string('percents', 'advworkform_numerrors', $i);
        }
        $mform->addElement('static', 'mappingzero', 0, get_string('percents', 'advworkform_numerrors', 100));
        for ($i = 1; $i <= $totalweight; $i++) {
            ...
            $selects[] = $mform->createElement('static', 'mapdefault__idx_'.$i, '',
                                        get_string('percents', 'advworkform_numerrors', floor(100 - $i * 100 / $totalweight)));
            $mform->addGroup($selects, 'grademapping'.$i, $i, array(' '), false);
            $mform->setDefault('map__idx_'.$i, '');
        }

        $mform->registerNoSubmitButton('noadddims');
        $mform->addElement('submit', 'noadddims', get_string('addmoredimensions', 'advworkform_numerrors',
                advwork_numerrors_strategy::ADDDIMS));
