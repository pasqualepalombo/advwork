...
/**
 * Provides the information to backup acc_mod grading strategy information
 */
class backup_advworkform_acc_mod_subplugin extends backup_subplugin {
    ...
    protected function define_advwork_subplugin_structure() {
        ...
        $subplugindimension = new backup_nested_element('advworkform_acc_mod_dimension', array('id'), array(
        ...
        $subplugindimension->set_source_table('advworkform_acc_mod', array('advworkid' => backup::VAR_ACTIVITYID));

        // id annotations
        $subplugindimension->annotate_ids('scale', 'grade');

        // file annotations
        $subplugindimension->annotate_files('advworkform_acc_mod', 'description', 'id');
    ...
    protected function define_referenceassessment_subplugin_structure() {
        return $this->dimension_grades_structure('advworkform_acc_mod_referencegrade');
    }
    ...
    protected function define_exampleassessment_subplugin_structure() {
        return $this->dimension_grades_structure('advworkform_acc_mod_examplegrade');
    }
    ...
    protected function define_assessment_subplugin_structure() {
        return $this->dimension_grades_structure('advworkform_acc_mod_grade');
    }
    ...
    private function dimension_grades_structure($elementname) {
        ...
        $subplugingrade->set_source_sql(
            "SELECT id, dimensionid, grade, peercomment, peercommentformat
               FROM {advwork_grades}
              WHERE strategy = 'acc_mod' AND assessmentid = ?",
