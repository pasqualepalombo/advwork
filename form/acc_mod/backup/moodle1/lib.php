...
/**
 * Conversion handler for the acc_mod grading strategy data
 */
class moodle1_advworkform_acc_mod_handler extends moodle1_advworkform_handler {

    /** @var used by {@link self::migrate_legacy_scales()} */
    private $newscaleids = array();

    /**
     * Converts <ELEMENT> into <advworkform_acc_mod_dimension>
     */
    public function process_legacy_element(array $data, array $raw) {
        ...
        $converted = (array)advworkform_acc_mod_upgrade_element($fakerecord, $newscaleid, 12345678);
        unset($converted['advworkid']);

        $converted['id'] = $data['id'];
        $this->write_xml('advworkform_acc_mod_dimension', $converted, array('/advworkform_acc_mod_dimension/id'));
     ...
     * can be used as a parameter for {@link advworkform_acc_mod_upgrade_element()}. Otherwise
     ...
     *   9 | Score out of 100
     *
     * @see advworkform_acc_mod_upgrade_scales()
    ...
    private function get_new_scale_definition($oldscaleid) {
        ...
        switch ($oldscaleid) {
        case 0:
            $data['name']  = get_string('scalename0', 'advworkform_acc_mod');
            $data['scale'] = implode(',', array(get_string('no'), get_string('yes')));
            break;
        case 1:
            $data['name']  = get_string('scalename1', 'advworkform_acc_mod');
            $data['scale'] = implode(',', array(get_string('absent', 'advworkform_acc_mod'),
                                                get_string('present', 'advworkform_acc_mod')));
            break;
        case 2:
            $data['name']  = get_string('scalename2', 'advworkform_acc_mod');
            $data['scale'] = implode(',', array(get_string('incorrect', 'advworkform_acc_mod'),
                                                get_string('correct', 'advworkform_acc_mod')));
            break;
        case 3:
            $data['name']  = get_string('scalename3', 'advworkform_acc_mod');
            $data['scale'] = implode(',', array('* ' . get_string('poor', 'advworkform_acc_mod'),
                                                '**',
                                                '*** ' . get_string('good', 'advworkform_acc_mod')));
            break;
        case 4:
            $data['name']  = get_string('scalename4', 'advworkform_acc_mod');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'advworkform_acc_mod'),
                                                '**',
                                                '***',
                                                '**** ' . get_string('excellent', 'advworkform_acc_mod')));
            break;
        case 5:
            $data['name']  = get_string('scalename5', 'advworkform_acc_mod');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'advworkform_acc_mod'),
                                                '**',
                                                '***',
                                                '****',
            ...
            break;
        case 6:
            $data['name']  = get_string('scalename6', 'advworkform_acc_mod');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'advworkform_acc_mod'),
                                                '**',
                                                '***',
                                                '****',
...
/**
 * Transforms a given record from advwork_elements_old into an object to be saved into advworkform_acc_mod
 *
 ...
 * @return stdclass to be saved in advworkform_acc_mod
 */
function advworkform_acc_mod_upgrade_element(stdclass $old, array $newscaleids, $newadvworkid) {
    $new = new stdclass();
