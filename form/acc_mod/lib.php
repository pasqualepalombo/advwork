...
/**
 * This file defines a class with acc_mod grading strategy logic
 *
 ...
 * @return bool
 */
function advworkform_acc_mod_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('advworkform_acc_mod', array('id' => $itemid ,'advworkid' => $advwork->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/advworkform_acc_mod/$filearea/$itemid/$relativepath";
...
/**
 * acc_mod grading strategy logic.
 */
class advwork_acc_mod_strategy implements advwork_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'advworkform_acc_mod', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new advwork_edit_acc_mod_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->acc_mod;  // records to be saved into {advworkform_acc_mod}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id         = $DB->insert_record('advworkform_acc_mod', $record);
            } else {
                // exiting field
                $DB->update_record('advworkform_acc_mod', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $PAGE->context, 'advworkform_acc_mod', 'description', $record->id);
            $DB->update_record('advworkform_acc_mod', $record);
        }
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'advworkform_acc_mod', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        $attributes = array('class' => 'assessmentform acc_mod');

        return new advwork_acc_mod_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public static function scale_used($scaleid, $advworkid=null) {
        ...
        if (!is_null($advworkid)) {
            $conditions['advworkid'] = $advworkid;
        }
        return $DB->record_exists('advworkform_acc_mod', $conditions);
    }
    ...
    public static function delete_instance($advworkid) {
        ...
        $DB->delete_records('advworkform_acc_mod', array('advworkid' => $advworkid));
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($PAGE->context->id, 'advworkform_acc_mod', 'description', $id);
            }
        }
        $DB->delete_records_list('advworkform_acc_mod', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link advwork_edit_acc_mod_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->acc_mod = array();        // records to be stored in {advworkform_acc_mod}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->acc_mod[$i]                     = new stdclass();
            $cook->acc_mod[$i]->id                 = $raw->{'dimensionid__idx_'.$i};
            $cook->acc_mod[$i]->advworkid         = $this->advwork->id;
            $cook->acc_mod[$i]->sort               = $i + 1;
            $cook->acc_mod[$i]->description_editor = $raw->{'description__idx_'.$i.'_editor'};
            $cook->acc_mod[$i]->grade              = $raw->{'grade__idx_'.$i};
            $cook->acc_mod[$i]->weight             = $raw->{'weight__idx_'.$i};
        }
        return $cook;
    }
    ...
    protected function get_current_assessment_data(stdclass $assessment) {
        ...
        $params = array('assessmentid' => $assessment->id, 'strategy' => 'acc_mod');
     ...
     *
     * In acc_mod grading strategy, scales are considered as grades from 0 to M-1, where M is the number of scale items.
     *
