 ...
 *
 * @package    advworkform_accumulative
 ...
 */
class moodle1_advworkform_accumulative_handler extends moodle1_advworkform_handler {

    /** @var used by {@link self::migrate_legacy_scales()} */
    private $newscaleids = array();

    /**
     * Converts <ELEMENT> into <advworkform_accumulative_dimension>
     */
    public function process_legacy_element(array $data, array $raw) {
        ...
        $converted = (array)advworkform_accumulative_upgrade_element($fakerecord, $newscaleid, 12345678);
        unset($converted['advworkid']);

        $converted['id'] = $data['id'];
        $this->write_xml('advworkform_accumulative_dimension', $converted, array('/advworkform_accumulative_dimension/id'));

        return $converted;
    }

    /**
     * If needed, creates new standard (global) scale to replace the legacy advwork one and returns the mapping
     *
     * If the given $oldscaleid represents a scale, returns array $oldscaleid => $newscaleid that
     * can be used as a parameter for {@link advworkform_accumulative_upgrade_element()}. Otherwise
     * this method returns empty array.
     *
     * In advwork 1.x, scale field in advwork_elements had the following meaning:
     ...
     *   9 | Score out of 100
     *
     * @see advworkform_accumulative_upgrade_scales()
    ...
    protected function get_new_scaleid($oldscaleid) {
        ...
        if ($oldscaleid >= 0 and $oldscaleid <= 6) {
            ...
            if (!isset($this->newscaleids[$oldscaleid])) {
                ...
                // inform the advwork instance that it should annotate the new scale
    ...
    /**
     * Returns a definition of a legacy advwork scale
     *
     * @see advworkform_accumulative_upgrade_scales
    ...
    private function get_new_scale_definition($oldscaleid) {
        ...
        switch ($oldscaleid) {
        case 0:
            $data['name']  = get_string('scalename0', 'advworkform_accumulative');
            $data['scale'] = implode(',', array(get_string('no'), get_string('yes')));
            break;
        case 1:
            $data['name']  = get_string('scalename1', 'advworkform_accumulative');
            $data['scale'] = implode(',', array(get_string('absent', 'advworkform_accumulative'),
                                                get_string('present', 'advworkform_accumulative')));
            break;
        case 2:
            $data['name']  = get_string('scalename2', 'advworkform_accumulative');
            $data['scale'] = implode(',', array(get_string('incorrect', 'advworkform_accumulative'),
                                                get_string('correct', 'advworkform_accumulative')));
            break;
        case 3:
            $data['name']  = get_string('scalename3', 'advworkform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('poor', 'advworkform_accumulative'),
                                                '**',
                                                '*** ' . get_string('good', 'advworkform_accumulative')));
            break;
        case 4:
            $data['name']  = get_string('scalename4', 'advworkform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'advworkform_accumulative'),
                                                '**',
                                                '***',
                                                '**** ' . get_string('excellent', 'advworkform_accumulative')));
            break;
        case 5:
            $data['name']  = get_string('scalename5', 'advworkform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'advworkform_accumulative'),
                                                '**',
                                                '***',
                                                '****',
            ...
            break;
        case 6:
            $data['name']  = get_string('scalename6', 'advworkform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'advworkform_accumulative'),
                                                '**',
                                                '***',
                                                '****',
...
/**
 * Transforms a given record from advwork_elements_old into an object to be saved into advworkform_accumulative
 *
 ...
 * @return stdclass to be saved in advworkform_accumulative
 */
function advworkform_accumulative_upgrade_element(stdclass $old, array $newscaleids, $newadvworkid) {
    $new = new stdclass();
    ...
    // are replaced with weight = 0. Legacy advwork did not store the raw weight but the index
    // in the array of weights (see $advwork_EWEIGHTS in advwork 1.x)
    // advwork 2.0 uses integer weights only (0-16) so all previous weights are multiplied by 4.
    switch ($old->weight) {
