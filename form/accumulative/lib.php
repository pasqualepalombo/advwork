 ...
 * @return bool
 */
function advworkform_accumulative_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('advworkform_accumulative', array('id' => $itemid ,'advworkid' => $advwork->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/advworkform_accumulative/$filearea/$itemid/$relativepath";
 ...
 */
class advwork_accumulative_strategy implements advwork_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'advworkform_accumulative', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new advwork_edit_accumulative_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->accumulative;  // records to be saved into {advworkform_accumulative}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id         = $DB->insert_record('advworkform_accumulative', $record);
            } else {
                // exiting field
                $DB->update_record('advworkform_accumulative', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $PAGE->context, 'advworkform_accumulative', 'description', $record->id);
            $DB->update_record('advworkform_accumulative', $record);
        }
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'advworkform_accumulative', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new advwork_accumulative_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id, d.grade, d.weight, s.scale
                  FROM {advworkform_accumulative} d
    ...
    public static function scale_used($scaleid, $advworkid=null) {
        ...
        return $DB->record_exists('advworkform_accumulative', $conditions);
    }
    ...
    public static function delete_instance($advworkid) {
        ...
        $DB->delete_records('advworkform_accumulative', array('advworkid' => $advworkid));
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($PAGE->context->id, 'advworkform_accumulative', 'description', $id);
            }
        }
        $DB->delete_records_list('advworkform_accumulative', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link advwork_edit_accumulative_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->accumulative = array();        // records to be stored in {advworkform_accumulative}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->accumulative[$i]                     = new stdclass();
            $cook->accumulative[$i]->id                 = $raw->{'dimensionid__idx_'.$i};
            $cook->accumulative[$i]->advworkid         = $this->advwork->id;
